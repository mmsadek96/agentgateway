...
describe('Governance Routes', () => {
  describe('GET /governance/info', () => {
    it('should return governance contract info', async () => {
      const res = await request(app).get('/governance/info');
      expect(res.status).toBe(200);
      expect(res.body.data).toHaveProperty('governor');
      expect(res.body.data).toHaveProperty('timelock');
      expect(res.body.data).toHaveProperty('token');
      expect(res.body.data).toHaveProperty('parameters');
      expect(res.body.data.parameters).toHaveProperty('votingDelay');
      expect(res.body.data.parameters).toHaveProperty('votingPeriod');
      expect(res.body.data.parameters).toHaveProperty('proposalThreshold');
      expect(res.body.data.parameters).toHaveProperty('quorum');
    });

    it('should include governed contracts list', async () => {
      const res = await request(app).get('/governance/info');
      expect(res.body.data).toHaveProperty('governedContracts');
      expect(Array.isArray(res.body.data.governedContracts)).toBe(true);
      expect(res.body.data.governedContracts.length).toBeGreaterThan(0);
    });

    it('should return correct contract addresses', async () => {
      const res = await request(app).get('/governance/info');
      expect(res.body.data.governor).toMatch(/^0x[0-9a-fA-F]{40}$/);
      expect(res.body.data.timelock).toMatch(/^0x[0-9a-fA-F]{40}$/);
      expect(res.body.data.token).toMatch(/^0x[0-9a-fA-F]{40}$/);
    });
  });

  describe('GET /governance/overview', () => {
    it('should return DeFi ecosystem overview', async () => {
      const res = await request(app).get('/governance/overview');
      expect(res.status).toBe(200);
      expect(res.body.data).toHaveProperty('enabled', true);
      expect(res.body.data).toHaveProperty('trustToken');
